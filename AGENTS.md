...
- **2026-09-24 (beşinci oturum, unreleased, main)** — Obfuscated-set round 3 (same PTer NZB, still "part 1/54"): the set's `hash.NN` numbering matches *neither* archive order nor old-style alphabetical, and `-hp`-encrypted headers would hide split flags anyway — name/role-based guessing is fundamentally unreliable. Probe gains a third stage: if neither end is a confirmed head, fetch every middle volume's first segment (JoinSet, pool-bounded) and sort by the **RAR5 volume number** from each MAIN header (`rar::sniff_rar_volume_number`) — authoritative, poster-proof, readable under `-hp`; missing/duplicate numbers → original order + accurate chain error. 5 new unit tests. Gate: 243 Rust + 152 Flutter. Android + macOS test packages built (`dist/android/ci-1416b5a/`, `dist/macos/`). Live verification still pending.
